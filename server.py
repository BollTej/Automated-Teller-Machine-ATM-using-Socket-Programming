...
with open(r'pin.txt', 'r') as f: #be sure to change this location of the file.
    lines = f.read().splitlines()
    last_line = lines[-1]
    pin=int(last_line)

#getiing balance of the user from external file
with open(r'bal.txt', 'r') as f:
    lines = f.read().splitlines()
    last_line = lines[-1]
    bal=int(last_line)
...
while True:
    ...
    while m!=5: #this loop will make sure that connection will be open till client demands to close it.
        ...
        with open(r'pin.txt','w') as f: #updating pin in external file
            f.write(str(pin)+'\n')
            f.close()
        ...
        with open(r'bal.txt','w') as f: #updating balance to the external file
            f.write(str(bal)+'\n')
            f.close()
        m=int(conne.recv(1024).decode())
